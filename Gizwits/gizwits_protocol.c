...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagpower_flag)
    {
        info->event[info->num] = EVENT_power_flag;
        info->num++;
        dataPoints->valuepower_flag = gizStandardDecompressionValue(power_flag_BYTEOFFSET,power_flag_BITOFFSET,power_flag_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagsmoke_threshold)
    {
        info->event[info->num] = EVENT_smoke_threshold;
        info->num++;
        dataPoints->valuesmoke_threshold = gizX2Y(smoke_threshold_RATIO,  smoke_threshold_ADDITION, exchangeBytes(issuedData->attrVals.valuesmoke_threshold));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuepower_flag != cur->valuepower_flag)
    {
        GIZWITS_LOG("valuepower_flag Changed\n");
        ret = 1;
    }
    if(last->valuesmoke_threshold != cur->valuesmoke_threshold)
    {
        GIZWITS_LOG("valuesmoke_threshold Changed\n");
        ret = 1;
    }
    ...
    if(last->valuesmoke_data != cur->valuesmoke_data)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuesmoke_data Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(power_flag_BYTEOFFSET,power_flag_BITOFFSET,power_flag_LEN,(uint8_t *)devStatusPtr,dataPoints->valuepower_flag);
    gizStandardCompressValue(smoke_BYTEOFFSET,smoke_BITOFFSET,smoke_LEN,(uint8_t *)devStatusPtr,dataPoints->valuesmoke);
    ...
    devStatusPtr->valuesmoke_threshold = exchangeBytes(gizY2X(smoke_threshold_RATIO,  smoke_threshold_ADDITION, dataPoints->valuesmoke_threshold)); 
    ...
    devStatusPtr->valuesmoke_data = exchangeBytes(gizY2X(smoke_data_RATIO,  smoke_data_ADDITION, dataPoints->valuesmoke_data)); 
