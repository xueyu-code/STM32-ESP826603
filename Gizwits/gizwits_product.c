...
extern int MQ2_flag;
extern uint16_t MQ2;
extern uint8_t data_temp,data_hum;
extern int MQ2_threshold;
...
int8_t gizwitsEventProcess(eventInfo_t *info, uint8_t *gizdata, uint32_t len)
{
  ...
  for(i=0; i<info->num; i++)
  {
    switch(info->event[i])
    {
      case EVENT_power_flag:
        currentDataPoint.valuepower_flag = dataPointPtr->valuepower_flag;
        GIZWITS_LOG("Evt: EVENT_power_flag %d \n", currentDataPoint.valuepower_flag);
        if(0x01 == currentDataPoint.valuepower_flag)
        {
          //user handle
        }
        else
...
void userHandle(void)
{
    ...
    currentDataPoint.valuehumidity = data_hum;//Add Sensor Data Collection
    currentDataPoint.valuetemperature =data_temp;//Add Sensor Data Collection
    currentDataPoint.valuesmoke_data = MQ2;//Add Sensor Data Collection
    currentDataPoint.valuesmoke =MQ2_flag;//Add Sensor Data Collection
		flag=currentDataPoint.valuepower_flag;
		MQ2_threshold=currentDataPoint.valuesmoke_threshold;
...
void userInit(void)
{
    ...
    /*
      ...
      currentDataPoint.valuesmoke_data = ;
